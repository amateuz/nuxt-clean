const createGame = (config = {}) => {
  ...
  function startGame() {
    ...
    gameObjects.push(
      that.add.image(
        startPos.images['room'].x,
        startPos.images['room'].y,
        'room'
      )
    )
    gameObjects.push(
      that.add.image(
        startPos.images['avatar'].x,
        startPos.images['avatar'].y,
        'avatar'
      )
    )
    gameObjects.push(
      that.add.image(
        startPos.images['items_popup'].x,
        startPos.images['items_popup'].y,
        'items_popup'
      )
    )
    ...
    checks.push(
      that.add.image(
        startPos.images['check_1'].x,
        startPos.images['check_1'].y,
        'check'
      )
    )
    checks.push(
      that.add.image(
        startPos.images['check_2'].x,
        startPos.images['check_2'].y,
        'check'
      )
    )
    checks.push(
      that.add.image(
        startPos.images['check_3'].x,
        startPos.images['check_3'].y,
        'check'
      )
    )
    ...
    checks.forEach((x) => {
      x.setScale(startPos.common.scale)
      x.setDepth(startPos.images['check_1'].depth)
    })

    items.push(
      that.add.image(
        startPos.images['item_1'].x,
        startPos.images['item_1'].y,
        'cloth_1_' + (startItems.head + 1).toString()
      )
    )
    items.push(
      that.add.image(
        startPos.images['item_2'].x,
        startPos.images['item_2'].y,
        'cloth_2_' + (startItems.bottom + 1).toString()
      )
    )
    items.push(
      that.add.image(
        startPos.images['item_3'].x,
        startPos.images['item_3'].y,
        'cloth_3_' + (startItems.top + 1).toString()
      )
    )
      ...
      x.on('pointerdown', function (pointer, localX, localY, event) {
        ...
        for (let j = 1; j <= 3; ++j) {
          ...
          var img = that.add.image(
            this.x,
            startPos.common.items.y - startPos.common.items.d * (j - 1),
            'cloth_' + c.toString() + '_' + j.toString()
          )
          img.setInteractive()
