const createGame = (config = {}) => {
  ...
  var gameImgs = ['fonur1', 'fonur2', 'fonur3', 'magnit1', 'magnit2', 'magnit3', 'tutorial', 'start', 'end', 'restart', 'exit', 'ur1_marker_kontur', 'ur2_marker_kontur', 'ur3_marker_kontur'];

  var points = [];

  var startPos = {
    'common': { 'scale': 0.625, 'width': 585, 'height': 800 }, 'images': {
  ...
  var remainLinks = [{ 'start': 0, 'end': 1 }, { 'start': 1, 'end': 2 }, { 'start': 2, 'end': 0 }];

  var levels = {
    'level1': {
      'points': [{ 'img': 'magnit1', 'x': 205, 'y': 375 }, { 'img': 'magnit2', 'x': 406, 'y': 383 }, { 'img': 'magnit3', 'x': 300, 'y': 485 }],
      'links': [{ 'start': 0, 'end': 1 }, { 'start': 1, 'end': 2 }, { 'start': 2, 'end': 0 }]
    },
    'level2': {
      'points': [{ 'img': 'magnit3', 'x': 210, 'y': 300 }, { 'img': 'magnit3', 'x': 397, 'y': 300 }, { 'img': 'magnit1', 'x': 222, 'y': 467 }, { 'img': 'magnit1', 'x': 374, 'y': 458 }, { 'img': 'magnit2', 'x': 299, 'y': 552 }],
      'links': [{ 'start': 0, 'end': 1 }, { 'start': 0, 'end': 2 }, { 'start': 1, 'end': 3 }, { 'start': 3, 'end': 4 }, { 'start': 2, 'end': 4 }]
    },
    'level3': {
      'points': [{ 'img': 'magnit3', 'x': 218, 'y': 326 }, { 'img': 'magnit3', 'x': 384, 'y': 327 }, { 'img': 'magnit2', 'x': 300, 'y': 415 }, { 'img': 'magnit1', 'x': 219, 'y': 507 }, { 'img': 'magnit1', 'x': 385, 'y': 507 }],
      'links': [{ 'start': 0, 'end': 1 }, { 'start': 1, 'end': 2 }, { 'start': 2, 'end': 0 }, { 'start': 2, 'end': 3 }, { 'start': 2, 'end': 4 }, { 'start': 3, 'end': 4 }, { 'start': 0, 'end': 3 }]
    },
  };
  ...
  function addImage(name, x, y, imgName) {
    ...
    let inm = imgName ?? (name in startPos.images && 'img' in startPos.images[name] ? startPos.images[name].img : name);
    ...
    if (name in startPos.images)
      img.setDepth(startPos.images[name].depth);
    img.setScale(scale);
    return img;
  }
  ...
  function startLevel() {
    ...
    if (!kontur) {
      kontur = addImage('tutorial', 300, 417, 'ur' + curLevel.toString() + '_marker_kontur');
    }
        ...
        if (last != null) {
          ...
          if (start != null) {
            ...
            var path = remainLinks.find(x => {
              ...
              return x.start === lastPointIdx && x.end === curIdx || x.start === curIdx && x.end === lastPointIdx;
            });
            if (path) {
  ...
  function create() {
    ...
    that.input.on('pointermove', function (pointer) {
